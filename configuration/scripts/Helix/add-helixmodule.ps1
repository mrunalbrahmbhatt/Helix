...
function Add-Projects
{
    ...
    Get-ChildItem -File -Path $ModulePath -Filter "*$csprojExtension" -Recurse | ForEach-Object { 
	
	$csprojFolder = Split-Path $_.FullName
	$assemblyInfoPath = Join-Path $csprojFolder "/Properties/AssemblyInfo.cs"
	
	if(Test-Path $assemblyInfoPath)
    {
	   Set-AssemblyGuid $assemblyInfoPath
	}
	
	$moduleNameInterface.AddFromFile("$($_.FullName)")}
